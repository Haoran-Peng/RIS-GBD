...
def Without_RIS_ipopt(Bandwidth, Noise, P_max, P_k, mu, PathLoss_UserBS, PathLoss_UserRIS, PathLoss_RISBS,
                   Num_User, Tx_antBS, RIS_Lnum, Tx_antRIS, Power_PU, PU_SINR_min, GBD_power_ini):
    
    #--------------------------------array setting-----------------------------------#
    # Noise_o=-174 #dBm  -174dBm/Hz
    ...
    def object_func_without(model):

        direct_link_sum = sum(abs(glvec_1[idx_c])**2 for idx_c in range(Tx_antBS)) 
        # print("D link : ", direct_link_sum[0])
        
        return (( direct_link_sum[0])*model.SUPower\
            /((direct_link_sum[0])*Power_PU + Noise))\
                /(mu*model.SUPower + P_k)
    ...
    #     return (1e20*(( direct_link_sum \
    #         + 2*sum(glvec_1[idx_m].real*RIS_part_real(model, idx_m)\
    #         + glvec_1[idx_m].imag*RIS_part_imag(model, idx_m) for idx_m in range(Tx_antBS))\
    #         + RIS_part_square(model))*Power_PU)) >= (1e20*(PU_SINR_min*((direct_link_sum \
    #             + 2*xini*sum(glvec_1[idx_m].real*RIS_part_real(model, idx_m)\
    #             + glvec_1[idx_m].imag*RIS_part_imag(model, idx_m) for idx_m in range(Tx_antBS))\
    #             + xini*RIS_part_square(model))*(model.SUPower) + Noise)))
                                                                         
    def constraint_04_without(model, Power_PU):   #--------------minimum primary receiver inequality---------------#
        
        direct_link_sum = sum(abs(glvec_1[idx_c])**2 for idx_c in range(Tx_antBS))

        return (1e15*(( direct_link_sum[0] )*Power_PU)) >= (1e15*(PU_SINR_min*((direct_link_sum[0] )*(model.SUPower) + Noise)))
    ...
    model = ConcreteModel(name="Without ris problem")
    ...
    model.SUPower = Var(bounds=(0.0001,P_max), within=NonNegativeReals, initialize = GBD_power_ini)
    ...
    model.cons.add(constraint_04_without(model, Power_PU))
    # for idx in range(Tx_antRIS):
    ...
    model.obj = Objective(expr=object_func_without, sense=maximize)
    ...
    # model.dual.display()
    model.cons.display()
    SINR_cons_ver =  Power_PU * (glvec_1.conj().T @ glvec_1)/ (primal_powerSU * (glvec_1.conj().T @ glvec_1) + Noise)
    
    print("--------Primal v2 Solved--------")
    print("primal status :", primal_prob_status)
    print("primal time :", PP_solvertime,"\n")
    print("power PU : ", Power_PU, "\nChannel gain : ", (glvec_1.conj().T @ glvec_1), "\nNoise : ", Noise,"\nPU SINR ver : ", SINR_cons_ver)
    
    
    return primal_powerSU, primal_prob_status, prob_lower_bound, PP_solvertime, PP_time
